...
def create_maze(rows, columns):
    ...
    maze = generate_maze(rows, columns, row_state, pin_location,
                         blank, slab, wall, corner)
...
def validate_dimensions(rows, columns):
    ...
    if not (MIN_ROWS - 1 <= rows <= MAX_ROWS - 1 and
            MIN_COLUMNS - 1 <= columns <= MAX_COLUMNS - 1):
        error_message()
...
def generate_maze(rows, columns, row_state, pin_location,
                  blank, slab, wall, corner):
    ...
    for row in range(1, rows // 2):
        ...
        for column in range(columns // 2):
            ...
            pin_location -= 1
            bottom_row += '#' if pin_location == 0 else blank
        if columns % 2:
            top_row += top_row[-1]
            bottom_row += blank
...
def generate_bottom_row(maze, wall, blank, slab):
    ...
    return last_row.translate(str.maketrans
                              ({wall: '+', blank: slab, '#': slab}))+"\n"
...
def error_message():
    print(
        ...
        f"       minimum columns >= {MIN_COLUMNS} "
        f"maximum columns <= {MAX_COLUMNS}\n"
